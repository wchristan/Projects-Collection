...
#define APP_VIRTUAL_WIDTH		(1480)					// This will be the effective x resolution regardless of actual screen/window res.
...
#define APP_PAD_EMUL_BUTTON_ALT_A		(VK_LBUTTON)
...
#define APP_PAD_EMUL_BUTTON_LEFT_SHOULDER	(VK_SHIFT)
...
#define IRAND	(rand() / (RAND_MAX))
#define IRAND_RANGE(_MIN_, _MAX_ ) (IRAND * ((_MAX_)-(_MIN_)) + (_MIN_))

#define PI		(3.14159265359f)
#define DOUBLE_PI (2.0f * PI)
#define HALF_PI (PI / 2.0f)
#define THREE_HALF_PI ((PI * 3.0f) / 2.0f)

//Custom definitions
#define RAD (PI/180.0f)	//Conversion to Radians
#define DEG (180.0f/PI)	//Conversion to Degrees

#define PLAYERDAMAGE 0	//Quick tag use for collision
#define ENEMYDAMAGE 1	//Quick tag use for collision
