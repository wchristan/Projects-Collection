...
{	
	...
	void Debug(int pos_, const char* msg_, float value_)
	{
		//Position on the screen
		float posX = 10;
		float posY = APP_VIRTUAL_HEIGHT - 80;
		posY -= pos_ * 20;
		...
		//Horizontal gap between text and value messages
		int gap = 100;

		char buffer[64];
		snprintf(buffer, sizeof(buffer), "%f", value_);

		Print(posX, posY, msg_, r, g, b);
		Print(posX + gap, posY, buffer, r, g, b);
	}

	void Debug(int pos_, const char* msg_, float v1_, float v2_)
	{
		//Position on the screen
		float posX = 10;
		float posY = APP_VIRTUAL_HEIGHT - 80;
		...
		char buffer[64];
		snprintf(buffer, sizeof(buffer), "%f", v1_);

		Print(posX, posY, msg_, r, g, b);
		Print(posX + gap, posY, buffer, r, g, b);

		snprintf(buffer, sizeof(buffer), "%f", v2_);
		Print(posX + (2 * gap), posY, "::", r, g, b);
		Print(posX + (2 * gap + 15), posY, buffer, r, g, b);
	}

	void PrintLabel(int posX, int posY, int gap, const char* msg_, int value_)
	{
		//Colour
		float r = 0.4f;
		float g = 0.8f;
